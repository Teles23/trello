...
function makeBoardTitle(title, id){
    return `<input type='text' value='${title}' class='inputEdit' id='input${id}' data-id='${id}' /> <span id='title${id}' class='boardTitle' data-id='${id}'>${title}</span> <i class='bi-pencil btnPadrao btnEditTitle' data-id='${id}'></i> <br style='clear:both' />`;
}

function makeTaskTitle(title, id){
  return `${title} <i class='bi-trash btnPadrao btnRemoveTask' data-id='${id}'></i>`;
}
      ...
      formItem.addEventListener("submit", function(e) {
        ...
        const createTask = async () => {
            try {
              ...
              await axios.post('php/index1.php', params, { params: { type: "create_task" }}, { headers: { 'Content-Type': 'application/json' }})
...
const sortBoards = async (sorted) => {
  ...
  try {
    ...
    await axios.post("php/index1.php", params, { params: { type: "sort_boards" }}, { headers: { 'Content-Type': 'application/json'}});
...
const updateBoard = async (boardId, boardTitle) => {
  ...
  try {
    ...
    await axios.post('php/index1.php', params, { params: { type: "update_board" }}, { headers: { 'Content-Type': 'application/json' }});
...
const updateTask = async (payload) => {
  ...
  try {
    ...
    await axios.post('php/index1.php', params, { params: { type: "update_task" }, }, { headers: { 'Content-Type': 'application/json' } });
  } catch (err) {
    console.error(err)
  }
...
const getBoards = async () => {
  try {
      await axios.get('php/index1.php', { params: { type: "read_boards" }, }, { headers: { 'Content-Type': 'application/json'}})
        .then(function (response) {
...
const getTasks = async () => {
    try {
        await axios.get('php/index1.php', { params: { type: "read_tasks" }, }, { headers: { 'Content-Type': 'application/json' } })
          .then(function (response) {
              tasks = response.data;
...
const deleteTask = async (taskId) => {
  ...
  try {
    ...
    await axios.get("php/index1.php", { params: { type: "delete_task", task_id: taskId}}, { headers: { 'Content-Type': 'application/json' }})
      .then(function (response){
...
addBoardDefault.addEventListener("click", function() {
  ...
  const createBoard = async () => {
    try {
      ...
      await axios.post("php/index1.php", null, { params: { type: "create_board" }}, { headers: { 'Content-type': 'application/json'}})
